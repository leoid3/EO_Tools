...
def show_sat(sat, ax, color):
    """
    Affiche les satellites sur l'orbite.
    """ 
    x_sat, y_sat, z_sat = sat.get_position()

    ax.plot(x_sat[len(x_sat)-1]/1000, y_sat[len(y_sat)-1]/1000, z_sat[len(z_sat)-1]/1000, '*', label=f'Satellite {sat.get_name()}',color=color, markersize=6)
